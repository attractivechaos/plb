...
sudoku.solve = function(e, s)
	...
	for i = 1, 9 do
		...
		for j = 1, 46656 do
			...
			then table.insert(z[i], e[j]) end
		end
		n[i] = bit.bor(bit.lshift(#z[i], 8), i)
	end
	table.sort(n)
	for i = 1, 9 do p[i], n[i] = 0, bit.band(n[i], 0xff) end
	x, i_ = {0, 0, 0}, 1
	while true do
		while i_ >= 1 and i_ <= 9 do
			...
			if p[i_] > 0 then
				local zip = zi[p[i_]]
				x[1], x[2], x[3] = bit.band(x[1], bit.bnot(zip[1])), bit.band(x[2], bit.bnot(zip[2])), bit.band(x[3], bit.bnot(zip[3]))
			end
			local j_ = #zi + 1
			for j = p[i_] + 1, #zi do
				if bit.band(x[1], zi[j][1]) == 0 and bit.band(x[2], zi[j][2]) == 0 and bit.band(x[3], zi[j][3]) == 0 then
					j_ = j; break
				end
			end
			if j_ <= #zi then
				x[1], x[2], x[3], p[i_], i_ = bit.bor(x[1], zi[j_][1]), bit.bor(x[2], zi[j_][2]), bit.bor(x[3], zi[j_][3]), j_, i_ + 1
			else p[i_], i_ = 0, i_ - 1 end
		end
		if i_ < 1 then break end
		local y = {}
		for j = 1, 9 do
			for k = 0, 31 do if bit.band(bit.rshift(z[n[j]][p[j]][1], k), 1) == 1 then y[k+1]  = n[j] end end
			for k = 0, 31 do if bit.band(bit.rshift(z[n[j]][p[j]][2], k), 1) == 1 then y[k+33] = n[j] end end
			for k = 0, 16 do if bit.band(bit.rshift(z[n[j]][p[j]][3], k), 1) == 1 then y[k+65] = n[j] end end
		end
		ret[#ret+1] = y
		i_ = i_ - 1
	end
